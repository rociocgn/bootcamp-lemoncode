...
// Creamos la interfaz para definir el tipo de retorno de la función (un objeto con dos propiedades numéricas)

const calculoDiasinIVA = (precioTotal: number, tipoIVA : number): CalculoDiasinIVA => {
    const precioBase = precioTotal / (1 + tipoIVA);
...
const selectTipoIVA = document.getElementById('tipoIVA') as HTMLSelectElement;
...
botonCalculo.addEventListener('click', () => {
    ...
    const tipoIVA = Number(selectTipoIVA.value);
    ...
    if (isNaN(tipoIVA) || tipoIVA < 0) {
        alert('Por favor, selecciona un tipo de IVA válido.');
        return; // Salimos de la función si el valor no es válido
    }
    
    //4. LLAMAMOS A LA FUNCIÓN DE CÁLCULO
    const resultado = calculoDiasinIVA(precioTotal, tipoIVA);
